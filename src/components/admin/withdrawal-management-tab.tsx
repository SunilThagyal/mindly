...
export default function WithdrawalManagementTab() {
  ...
  const handleConfirmRejection = async (reason: string) => {
    ...
    setUpdatingStatus(requestToReject.id!);
    const adminRejectionNotes = reason.trim() || "No specific reason provided.";

    try {
        await runTransaction(db, async (transaction) => {
            const requestDocRef = doc(db, 'withdrawalRequests', requestToReject.id!);
            const userDocRef = doc(db, 'users', requestToReject.userId);

            transaction.update(userDocRef, {
                virtualEarnings: increment(requestToReject.amount)
            });
        ...
        const notificationRef = collection(db, 'users', requestToReject.userId, 'notifications');
        await addDoc(notificationRef, {
            type: 'withdrawal_rejected',
            withdrawalAmount: requestToReject.amount,
            adminNotes: adminRejectionNotes,
        ...
        toast({ title: 'Request Rejected', description: `Amount of $${requestToReject.amount.toFixed(2)} refunded.`, variant: 'success' });
    ...
    <>
      ...
      {requestToReject && (
          <RejectionReasonDialog
              ...
              isSubmitting={updatingStatus === requestToReject.id}
          />
      )}
    </>
  );
