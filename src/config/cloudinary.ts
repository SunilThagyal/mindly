...
if (typeof window !== 'undefined') { // Only run this warning logic in the browser
  if (!CLOUDINARY_CLOUD_NAME || !CLOUDINARY_UPLOAD_PRESET || CLOUDINARY_UPLOAD_PRESET === 'your_unsigned_upload_preset_name') {
    ...
    const isMissingUploadPreset = !CLOUDINARY_UPLOAD_PRESET || CLOUDINARY_UPLOAD_PRESET === 'your_unsigned_upload_preset_name';
    ...
    if (isMissingUploadPreset) {
      warningMessage += "- NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET environment variable is not set or is using the placeholder value.\n";
    }
    ...
    warningMessage += "\nImage uploads within the blog editor (both cover image and in-content images) will NOT work until these are correctly configured in your .env file (e.g., .env.local).\n" +
      ...
      `Then, update your .env file with your actual values. Example:\n` +
      `NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME=${CLOUDINARY_CLOUD_NAME || 'your_cloud_name'}\n` +
      `NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET=your_actual_unsigned_upload_preset_name\n\n` +
